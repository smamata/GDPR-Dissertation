...
type Variant = "basic" | "optimized" | "minimal";

async function deploy(variant: Variant) {
  const name = variant === "basic" ? "ConsentBasic" : variant === "optimized" ? "ConsentOptimized" : "ConsentMinimalEvent";
  const Factory = await ethers.getContractFactory(name);
  const contract = await Factory.deploy();
  await contract.waitForDeployment();
  return { name, address: await contract.getAddress(), instance: contract as any };
}

async function measureGas<T>(fn: () => Promise<T>) {
  const tx = await fn();
...
async function run() {
  ...
  const variant = (process.env.VARIANT || "basic") as Variant;

  const { name, address, instance } = await deploy(variant);
  console.log(`Variant ${name} deployed at ${address}`);

  const [signer, ...others] = await ethers.getSigners();
